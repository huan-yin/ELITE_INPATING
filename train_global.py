...
from diffusers import AutoencoderKL, DDPMScheduler, UNet2DConditionModel, LMSDiscreteScheduler, StableDiffusionInpaintPipeline
...
from datasets import OpenImagesDataset, MvtecDataset, MvtecNewDataset
...
def tensor2image(image):
    ...
    return Image.fromarray(image).resize((512, 512))

def tensor2refimage(ref_image_tensor):
    ref_image_tensor = ref_image_tensor.detach().cpu()
    mean = (0.48145466, 0.4578275, 0.40821073)
    std = (0.26862954, 0.26130258, 0.27577711)
    mean_tensor = torch.tensor(mean).unsqueeze(1).unsqueeze(2)
    std_tensor = torch.tensor(std).unsqueeze(1).unsqueeze(2)
    denormalized = ref_image_tensor * std_tensor + mean_tensor  # 形状保持(C, H, W)
    
    # 2. 裁剪到0-1范围（避免归一化导致的数值溢出）
    denormalized_clamped = denormalized.clamp(0.0, 1.0)
    
    # 3. 转换为0-255的uint8（逆ToTensor的数值缩放）
    tensor_0_255 = (denormalized_clamped * 255).to(torch.uint8)
    
    # 4. 调整维度为(H, W, C)（逆ToTensor的维度转换）
    tensor_transposed = tensor_0_255.permute(1, 2, 0)  # 从(C, H, W)→(H, W, C)
    
    # 5. 转为numpy数组并生成PIL图像
    img_array = tensor_transposed.numpy()
    return Image.fromarray(img_array).resize((512, 512))

def tensor2mask(mask_tensor):
    mask_np = mask_tensor.detach().cpu().numpy()  # 形状[1, H, W]
    
    # 2. 移除多余的通道维度（从[1, H, W]→[H, W]）
    mask_np = mask_np.squeeze(0)  # 形状[H, W]

    # 3. 扩展为三通道（RGB）：每个通道值与单通道相同
    # 先增加一个通道维度[H, W, 1]，再重复为[H, W, 3]
    mask_3ch = np.repeat(mask_np[:, :, np.newaxis], 3, axis=2)  # 形状[H, W, 3]
    
    # 4. 转换为0-255的uint8（PIL图像要求）
    mask_3ch = (mask_3ch * 255).astype(np.uint8)  # 0→0，1→255
    
    # 5. 转为PIL图像（默认是RGB模式）
    return Image.fromarray(mask_3ch).resize((512, 512))
...
def validation(example, tokenizer, image_encoder, text_encoder, unet, mapper, vae, device, guidance_scale, token_index='full', seed=None):
    ...
    uncond_input = tokenizer(
        [''] * example["pixel_values_image"].shape[0],
        padding="max_length",
    ...
    if seed is None:
        latents = torch.randn(
            (example["pixel_values_image"].shape[0], 4, 64, 64)
        )
    else:
        generator = torch.manual_seed(seed)
        latents = torch.randn(
            (example["pixel_values_image"].shape[0], 4, 64, 64), generator=generator,
        )

    latents = latents.to(example["pixel_values_ref_image"])
    scheduler.set_timesteps(100)
    latents = latents * scheduler.init_noise_sigma
   
    placeholder_idx = example["index"]

    masked_image_latents = vae.encode(example["pixel_values_masked_image"].reshape(example["pixel_values_image"].shape)).latent_dist.sample().detach()
    masked_image_latents = masked_image_latents * 0.18215


    pixel_values_mask = F.interpolate(example["pixel_values_mask"], (64, 64), mode='bilinear')
    

    image = F.interpolate(example["pixel_values_ref_image"], (224, 224), mode='bilinear')
    ...
    ret_pil_images = [tensor2image(image) for image in images]
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(mapper):
                ...
                latents = vae.encode(batch["pixel_values_image"]).latent_dist.sample().detach()
                latents = latents * 0.18215
                masked_latents = vae.encode(batch["pixel_values_masked_image"]).latent_dist.sample().detach()
                masked_latents = masked_latents * 0.18215
                pixel_values_mask = F.interpolate(batch["pixel_values_mask"], (64, 64), mode='bilinear')
                ...
                latent_model_input = torch.cat([noisy_latents, pixel_values_mask, masked_latents], dim=1)

                placeholder_idx = batch["index"]
                image = F.interpolate(batch["pixel_values_ref_image"], (224, 224), mode='bilinear')
            ...
            if accelerator.sync_gradients:
             ...
             # save_progress(mapper, accelerator, args, global_step)
                    syn_images = validation(batch, tokenizer, image_encoder, text_encoder, unet, mapper, vae, batch["pixel_values_ref_image"].device, 5)
                    gt_images = [tensor2image(img) for img in batch["pixel_values_image"]]
                    ref_images = [tensor2refimage(img) for img in batch["pixel_values_image"]]
                    mask_images = [tensor2mask(img) for img in batch["pixel_values_mask"]]
                    img_list = []
                    for syn, gt, ref, mask in zip(syn_images, gt_images, ref_images, mask_images):
                        img_list.append(np.concatenate((np.array(mask), np.array(gt), np.array(ref), np.array(syn)), axis=1))
