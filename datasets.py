...
def get_mask_path(image_path):
    ...
    # 构造掩码文件名（在原文件名后添加"_mask"）
    mask_file_name = f"{base_name}_mask{ext}"
    
    # 将目录从"images"替换为"masks"
    mask_dir = dir_name.replace("images", "masks")
...
class MvtecDataset(Dataset):
    ...
    def process_pil_image(self, image):
        image_np = np.array(image)
        img = cv2.resize(image_np, (self.size, self.size), interpolation=cv2.INTER_CUBIC)
        img = np.array(img).astype(np.float32)
        img = img / 127.5 - 1.0
        return torch.from_numpy(img).permute(2, 0, 1)
    

    def process_pil_mask(self, mask):
        mask = np.array(mask)
    ...
    def __getitem__(self, i):
        ...
        pixel_values_image = self.process_pil_image(image)

        pixel_values_mask = self.process_pil_mask(mask)

        pixel_values_masked_image = pixel_values_image * (pixel_values_mask < 0.5)

        

        example["pixel_values_image"] = pixel_values_image
        example["pixel_values_mask"] =  pixel_values_mask
        example["pixel_values_masked_image"] = pixel_values_masked_image

        ref_image_np = np.array(image)
        ref_image_tensor = self.random_trans(image=ref_image_np)
        ref_image_tensor = Image.fromarray(ref_image_tensor["image"])
        example["pixel_values_ref_image"] = self.get_tensor_clip()(ref_image_tensor)
...
class MvtecNewDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            example["pixel_values_image"] = self.process(image_tensor)
            ...
            pixel_values_masked_image = example["pixel_values_image"] * (pixel_values_mask < 0.5)
            ...
            example["pixel_values"] = torch.zeros((3, 512, 512))
            example["pixel_values_ref_image"] = torch.zeros((3, 224, 224))
            with open('error.txt', 'a+') as f:
                f.write(str(e) + '\n')
