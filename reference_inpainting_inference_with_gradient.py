...
def compute_loss(latents, ref_image_latents, mask):

    mse_loss = F.mse_loss(latents, ref_image_latents, reduction='none')
...
with torch.no_grad():
    ...
    ref_image_latents = vae.encode(F.interpolate(pixel_values_ref_image, (512, 512), mode='bilinear')).latent_dist.sample().detach()
    ...
    scale_range = np.linspace(scale_range[0], scale_range[1], len(scheduler.timesteps))
    for i, t in enumerate(tqdm(scheduler.timesteps)):
        with torch.enable_grad():
            latents = latents.clone().detach().requires_grad_(True)
            loss = compute_loss(latents, ref_image_latents, pixel_values_mask)
            print(f'Iteration {i} | Loss: {loss:0.4f}')
            if loss != 0:
                latents = update_latent(latents=latents, loss=loss, step_size=scale_factor * np.sqrt(scale_range[i]))
...
Image.fromarray(concat).save(os.path.join(save_dir, f'generated_image_with_gradient.png'))